...
namespace SampleSitecoreProject
{
    public class SampleSitecoreLogic
    {
        ...
        {
            Item folderTemplate = Context.Database.GetItem(TemplateIDs.Folder);
            Item placemarkTemplate = Context.Database.GetItem("/sitecore/templates/User Defined/Kml/Placemark");
            Item contentRoot = Context.Database.GetItem(ItemIDs.ContentRoot);

            Item importRoot = contentRoot.Add(string.Format("Imported content {0}", timeStamp.ToString("yyyy MM dd")),
                new TemplateItem(folderTemplate));
            ...
            foreach (XElement placemarkElement in kmlDocument.Descendants(XName.Get("Placemark", kmlNamespace)))
            {
                ...
                {
                    Item placemarkItem = importRoot.Add(ItemUtil.ProposeValidItemName(name.Value.Replace(":", "")),
                        new TemplateItem(placemarkTemplate));
                    using (new EditContext(placemarkItem))
                    {
                        ...
                        if (point != null)
                        {
                            ...
                            {
                                string[] splitCoordinates = coordinates.Value.Split(new[] {','},
        ...
        {
            var imageFile = new FileInfo(sampleImage);
            Item parentItem = Context.Database.GetItem("/sitecore/media library");

            var mediaCreatorOptions = new MediaCreatorOptions();
            mediaCreatorOptions.Database = Context.Database;
            mediaCreatorOptions.Language = Context.Language;
            mediaCreatorOptions.Versioned = false;
            mediaCreatorOptions.Destination = string.Format("{0}/{1}", parentItem.Paths.FullPath,
                ItemUtil.ProposeValidItemName(Path.GetFileNameWithoutExtension(sampleImage)));
            mediaCreatorOptions.FileBased = Settings.Media.UploadAsFiles;
